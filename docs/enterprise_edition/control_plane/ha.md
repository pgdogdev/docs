...
The [control plane](index.md) supports deploying multiple instances on different machines. In case of hardware failure, its responsibilities can be taken over by a standby instance with almost no downtime.

## How it works

The leader instance, elected using Raft, performs all actions and monitors PgDog instances connected to it. Standby instances are also reachable via HTTP and will forward all API requests to the leader automatically.
...
If you're using our [Helm chart](installation.md) to deploy the control plane, Raft can be easily enabled with the following configuration:
...
!!! note "Number of nodes"
    ...
    For Raft to work correctly (and optimally), configure three (3) nodes operating on different
    hardware instances. The number of nodes must always be odd (e.g., 3, 5, 7). The more nodes that are part
